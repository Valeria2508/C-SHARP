...
//calificaciones ("de estudiantes separadas por coma

Console.WriteLine("Escribe las notas del estudiante separadas por ,");
string notas = Console.ReadLine();
List<string> notasSeparadas = notas.Split(',' , ' ', StringSplitOptions.TrimEntries).ToList();

foreach (var nota in notasSeparadas)
{
    Console.WriteLine($"Nota: {nota}");
}

var notasSeparadasInt = notasSeparadas.Select(c => Convert.ToDouble(c, CultureInfo.InvariantCulture)).ToList();

var promedio = notasSeparadasInt.Average();
Console.WriteLine($"El promedio de tus calificaciones es: {promedio}");